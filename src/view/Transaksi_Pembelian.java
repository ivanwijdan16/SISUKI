...
import dao.DAO_Pembelian;
import dao.DAO_DetPembelian;
import dao.DAO_Laporan;
import dao.DAO_SemPembelian;
...
import tablemodel.TableMod_Pembelian;
import tablemodel.TableMod_DetPembelian;
import tablemodel.TableMod_SemPembelian;


public class Transaksi_Pembelian extends javax.swing.JPanel {

    private Service_Pembelian servis_masuk = new DAO_Pembelian();
    private Service_DetPembelian servis_det = new DAO_DetPembelian();
    private Service_SemPembelian servis_sem = new DAO_SemPembelian();
    
    private TableMod_Pembelian tblModel_masuk = new TableMod_Pembelian();
    private TableMod_DetPembelian tblModel_det = new TableMod_DetPembelian();
    private TableMod_SemPembelian tblModel_sem = new TableMod_SemPembelian();
    ...
    public Transaksi_Pembelian(String Id) {
        initComponents();
        lb_id.setText(Id);
...
//    }
    ...
    private void initComponents() {
        ...
        jLabel10.setText("Detail Transaksi Pembelian");
        ...
        jLabel2.setText("Tambah Transaksi Pembelian");
        ...
        lb_total.setText("Total Pembelian");
    ...
    private void btn_tambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_tambahActionPerformed
        ...
        lb_total.setText(det_masuk.getSubtotal_pembelian().toString());
        setTanggal();
    ...
    private void btn_tambahSementaraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_tambahSementaraActionPerformed
        ...
        det_masuk.setJml_pembelian(jumlah);
        det_masuk.setSubtotal_pembelian(subtotal);
        //psn.setTotal_pesan(tobar);
        
        smt.setMod_barang(brg);
        smt.setMod_detbeli(det_masuk);
    ...
    private void btn_hapusSementaraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_hapusSementaraActionPerformed
        ...
        if (index!=-1) {
            Model_SemPembelian model = tblModel_sem.getData(tbl_sementara.convertRowIndexToModel(index));
    ...
    private void btn_printActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_printActionPerformed
        ...
        servis_lap.transaksiPembelian(no);
    ...
    private void tbl_barangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_barangMouseClicked
        ...
        List<Model_DetPembelian> list = servis_det.getData(id);
...
//            t_stok.setText              (tbl_barang.getModel().getValueAt(row, 7).toString());
    ...
    private void loadData() {
        ...
        List<Model_Pembelian> list = servis_masuk.getData();
        tblModel_masuk.setData(list);
    }

    private void loadDataSementara() {
        List<Model_SemPembelian> list = servis_sem.getData();
    ...
    private void hapusData() {
        ...
        if (index!=-1) {
            Model_Pembelian brg = tblModel_masuk.getData(tbl_barang.convertRowIndexToModel(index));
            if (JOptionPane.showConfirmDialog(null, "Yakin data akan dihapus?","Konfirmasi", JOptionPane.OK_CANCEL_OPTION)== JOptionPane.OK_OPTION) {
    ...
    private void tampilPanel() {
        ...
        mainPanel.add(new Transaksi_Pembelian(Id));
        mainPanel.repaint();
        mainPanel.revalidate();
    }
    ...
    private void simpanData() {
            ...
            Model_Barang brg = new Model_Barang();
            Model_Pembelian masuk = new Model_Pembelian();
            Model_DetPembelian det = new Model_DetPembelian();

            //Simpan Tabel Pemesanan
            masuk.setId_pembelian(no);
            masuk.setTgl_pembelian(tgl);
            masuk.setTotal_pembelian(total);
            dis.setId_supplier(id_dis);
            pgn.setId_user(id_peng);
            
            masuk.setMod_supplier(dis);
            masuk.setMod_user(pgn);
            
            //Simpan Tabel Detail Pemesanan
            det.setMod_beli(masuk);
    ...
    private void perbaruiDataSementara() {
        ...
        if (index!=-1) {
            Model_SemPembelian model = tblModel_sem.getData(tbl_sementara.convertRowIndexToModel(index));
                ...
                Model_Barang brg = new Model_Barang();
                Model_DetPembelian det = new Model_DetPembelian();
                Model_SemPembelian sem = new Model_SemPembelian();

                brg.setId_barang(kode);
                brg.setNama_barang(nama);
                brg.setHarga(harga);
                det.setJml_pembelian(jumlah);
                det.setSubtotal_pembelian(subtotal);

                sem.setMod_barang(brg);
                sem.setMod_detbeli(det);

                servis_sem.perbaruiData(sem);
                tblModel_sem.perbaruiData(index, sem);
                servis_det.sumTotal(det);
                lb_total.setText(det.getSubtotal_pembelian().toString());
                loadDataSementara();
                bersihBarang();
                resetForm();
    ...
    private void pencarian() {
        List<Model_Pembelian> list = servis_masuk.pencarian(t_cari.getText());
        tblModel_masuk.setData(list);
    }
